...
public class AppointmentNotificationConsumer {
    ...
    private static final String CONSUMER_NAME = "AppointmentNotificationConsumer";

    private final ObjectMapper objectMapper;
    private final AppointmentNotificationRecordMapper appointmentNotificationRecordMapper;
    ...
    public AppointmentNotificationConsumer(
            ...
            AppointmentNotificationRecordMapper appointmentNotificationRecordMapper,
        ...
        this.appointmentNotificationRecordMapper = appointmentNotificationRecordMapper;
    ...
    public void consume(Message message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long deliveryTag)
        ...
        try {
            ...
            persistNotificationRecord(eventId, event);
            saveSuccessLog(eventId, existingLog);
            channel.basicAck(deliveryTag, false);
        } catch (Exception e) {
            log.error("Failed to consume appointment notification event, eventId={}", eventId, e);
    ...
    private void persistNotificationRecord(Long eventId, AppointmentDomainEvent event) {
        AppointmentNotificationRecord record = new AppointmentNotificationRecord();
        record.setEventId(eventId);
        record.setAppointmentId(event.getAppointmentId());
        record.setPatientId(event.getPatientId());
        record.setNotificationType(event.getEventType());
        record.setStatus(CONSUME_STATUS_SUCCESS);
        record.setSentAt(LocalDateTime.now());
        try {
            appointmentNotificationRecordMapper.insert(record);
        } catch (DuplicateKeyException e) {
            log.info("Appointment notification side effect already persisted, eventId={}", eventId);
        }
    }
