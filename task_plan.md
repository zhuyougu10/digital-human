...
### 0. 当前后端流程测试
- [x] 无缓存重新构建 Docker 服务镜像。
- [x] 启动后端栈并等待服务可用。
- [x] 准备患者/医生测试账号、医生档案和可预约号源。
- [x] 执行患者登录、数字人导诊对话、患者预约与预约记录校验。
- [x] 执行医生端登录，校验医生预约列表和患者导诊摘要。
- [x] 输出本次测试报告。
...
### 6. 导诊 Agent 确定性重构
- [x] 移除导诊 Agent 对大模型工具编排的依赖，改为后端确定性状态机。
- [x] 落地“先问预约时间 -> 找医生 -> 用户选医生 -> 确认预约”的固定流程。
- [x] 补充导诊状态机、ChatService 分流和 Agent 契约单测。
- [x] 使用指定患者 token 完成真实网关复测，并确认医生端预约和摘要可见。
