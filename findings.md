...
- [2026-03-20] **[修正前述判断]** 用户提供的最新真机报错 `TypeError: Failed to fetch` 说明问题不是自动播放策略，也不是 UniApp 原生播放方案更稳，而是音频 GET 请求本身被浏览器拒绝。
- [2026-03-20] 进一步代码复核确认真实根因在后端 CORS：`medical-ai/medical-service/medical-ai-service/src/main/java/com/medical/ai/controller/ChatController.java` 的 `getTtsAudio()` 手动返回 `Access-Control-Allow-Origin: *`，同时 Gateway `application.yml` 已配置全局 CORS 且 `allowCredentials: true`。浏览器收到重复且冲突的 `Access-Control-Allow-Origin` 头后，会把音频 fetch 直接拒绝为 `TypeError: Failed to fetch`。
- [2026-03-20] 最小正确修复不是继续改前端地址解析，而是删除 `ChatController#getTtsAudio()` 里的手动 CORS 头，让 Gateway 统一输出 CORS 响应；该修复已在主工作区落地，并用 `mvn compile -pl medical-service/medical-ai-service -am -f medical-ai/pom.xml -q` 验证通过。
